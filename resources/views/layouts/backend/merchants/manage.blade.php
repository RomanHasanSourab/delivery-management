...
@section('content')
              ...
              <div class="card-body">
                {{-- @if(auth()->user()->role_id == 1) --}}
                  <div class="float-left">
                      <a class="btn btn-primary pull-right" href="{{route('merchants.index')}}">Back to MERCHANTS List</a>
                  </div>
                  <div class="float-right">
                      <button type="button" class="btn btn-rose" data-toggle="modal" data-target=".agent-tag" onclick="get_tag_id()">Assign to Agent</button>
                  </div>
              {{-- @endif --}}
                  ...
                  <nav class="navbar navbar-expand-lg">
                      ...
                      <li style="list-style: none;" class="nav-item">
                        <form action="{{route('delivery.invoice')}}" method="POST">
                            @csrf
                            <input type="text" name="delivery_id" id="invoice" value="" hidden>
                            <input type="text" name="user_id" value="{{$id}}" hidden>
                            <button class="btn btn-primary" onclick="get_invoice_id()">Generate Invoice</button>
                        </form>
                    </li>
                      @if(auth()->user()->id == 1)
                      <li style="list-style: none;" class="nav-item">
                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target=".delete" onclick="get_delete_id()">Delete</button>
                    </li>
                      @endif
                  </nav>

                  <table class="table table-bordered" class="display nowrap" id="manage-deliveries-admin"  width="100%">
                      <thead>
    ...
    <div class="modal fade agent-tag" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
        {{-- <div class="modal-dialog modal-lg modal-dialog-centered"> --}}
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="{{route('agent.tag')}}" method="POST">
                        @csrf
                        <input type="text" name="delivery_id" id="agent_tag" value="" hidden>
                        <div class="row">
                            <div class="col-md-10">
                                <div class="form-group">
                                    <select style="width: 100%" class="select2" name="agent_id" required>
                                        @foreach ($agents as $agent)
                                            <option></option>
                                            <option value="{{$agent->id}}">{{$agent->code}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
  ...
  <script>
      $(function() {
          ...
          $('#manage-deliveries-admin').DataTable({
            processing: true,
            serverSide: true,
            // responsive: true,
            scrollX: true,
            ...
            // paging: false,
              ajax: {
                    url: '{{ url('manage-deliveries-admin') }}',
                    data: {'user_id': {{$id}}},
                    type: 'post'
                },
              columns: [
    ...
    function get_invoice_id() {
        ...
        if (selected_item.length > 0) {
            document.getElementById("invoice").value = selected_item;
        } else {
                alert("Please Select an item");
        }
    }
